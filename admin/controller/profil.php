...
class profil extends Controller {
	...
	public function input_sejarah(){
		...
		echo "<script>alert('Data Berhasil Disimpan');document.location='sejarah';</script>";
	...
	public function input_tupoksi(){
			...
			try
			   {
			   		if(isset($x) && count($x) != 0)
			   		{
						...
						$input_data_peraturan = $this->models->inputData_tupoksi($x['id'],$x['title'],$x['content'],$x['categoryid'],$x['articletype'],
		...
		echo "<script>alert('Data Berhasil Disimpan');document.location='tupoksi';</script>";
	}

...
class m_slide extends Database {
	...
	function viewUser($table_user){
		$sql = "SELECT * FROM ".$table_user." WHERE n_status !=2 ";
		// pr($sql);
		$result = $this->fetch($sql,1);
		if ($result) return $result;
		return false;
	...
	function inputDataFile($id,$title,$image,$categoryid,$typeFile,$createdate,$postdate,$fromwho,$authorid,$n_status,$link)
	{
		
		if($id){
			$query = "UPDATE dtataruang_news_content SET 
					title='".$title."',image='".$image."',categoryid='".$categoryid."',tags='".$typeFile."',
					createdate='".$createdate."',postdate='".$postdate."',fromwho='".$fromwho."',authorid='".$authorid."',n_status='".$n_status."',brief='".$link."' WHERE id='".$id."'";
			$result['data'] = $this->query($query);
			$result['param'] = 'U';
			$result['id'] = $id;
		}else{
		
			$query = "INSERT INTO dtataruang_news_content (title,image,categoryid,tags,createdate,postdate,fromwho,authorid,n_status,brief) 
					VALUES ('".$title."','".$image."','".$categoryid."','".$typeFile."','".$createdate."','".$postdate."','".$fromwho."','".$authorid."','".$n_status."','".$link."')";
			$result['data'] = $this->query($query);
			$result['param'] = 'I';

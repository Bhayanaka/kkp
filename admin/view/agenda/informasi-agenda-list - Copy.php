				...
				<a href="<?=$basedomain?>agenda" class="sub_menu_in on">Agenda</a>
				...
				<h4 class="title_sub_menu_in">List Agenda</h4>
				<?php 
				if($_SESSION['user']['usertype'] == '1' || $_SESSION['user']['usertype'] == '2' || $_SESSION['user']['usertype'] == '3' ){
				?>
				<div class="row-fluid" >
					<div class="span6"><a href="<?=$basedomain?>agenda/add" class="btn btn-primary">Tambah Data</a></div>
				</div>
				<?php
				}
				echo "&nbsp;";
				...
				<table class="table table-striped" id="tableajax">
					...
					<tr>
						...
						<td>
							...
							?>
								<a href="<?=$basedomain?>agenda/change_status/?id=<?=$val['id']?>&n_status=<?=$val['n_status']?>" class="icon-globe" title="Publish"></a>&nbsp;&nbsp;&nbsp;
								<a href="<?=$basedomain?>agenda/add/?id=<?=$val['id']?>" class="icon-edit" title="Edit"></a>&nbsp;&nbsp;&nbsp;
								<a href="<?=$basedomain?>agenda/delete/?id=<?=$val['id']?>" class="icon-trash" title="Hapus" onclick="return confirm('Hapus Data');"></a>
							<?php

...
// pr($data);
?>				
<h4 class="title_sub_menu_in">Tambah Peta Struktur Ruang</h4>
<div class="new_input">
<form method="post" action="<?=$basedomain?>sig/input_pola_struktur" enctype = "multipart/form-data">
<table >
	<tr>
		<td>Judul</td>
		<td>&nbsp;</td>
		<td><input type="text" name="title" required="required" value="<?=(isset($result[0]['title']) ? $result[0]['title'] : '')?>"/></td>
		<td>&nbsp;</td>
...
<textarea style="width:100%;height:200px;" name="content" class="ckeditor"><?=(isset($result[0]['content']) ? $result[0]['content'] : '');?></textarea>
		...
		<input type="hidden" name="categoryid" id="categoryid"  value="<?=(isset($result[0]['categoryid']) ? $result[0]['categoryid'] : '34') ?>"/>

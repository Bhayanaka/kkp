...
*/		
	...
	$title_page=array('Aplikasi','SIG Pengelolaan Pesisir dan Pulau - Pulau Kecil','Peta Pola Ruang','Kabupaten - Kota Rinci');
...
?>
		<div class="row-fluid ">
			...
			<div class="span10 bg_putih body">
				<div class="row-fluid">
				  <div class="span8">
						<h3>Peta Struktur Ruang</h3>
						<form method="post" action="<?=$basedomain?>minapolitan/searchSigStruktur" class="form-inline" >
							<div class="controls">
								<?php
									provLocation($provinsi,'on','');
									echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
									kabLocation('off',$list,$kabupaten);
									echo "&nbsp;";
									?>
								<input class="" type="hidden" name="type" value="<?=$type?>">
								<input class="" type="hidden" name="categoryid" value="34">
								<br /><br />
								...
								if ($keyword !=''){
								?>
								+ <font color="blue"><i><?=$keyword?></i></font>
								<?php
								}
								if ($prov !=''){
								?>
						...
						<table class="peraturan" border="0">
							...
							<tr>
								...
								<td>:</td>
								<td><?php foreach ($provinsi as $value){
							...
							<tr>
								<td>&nbsp;</td>
								<td>:</td>
								<td><?php foreach ($kabupaten as $kab){
									if($kab['kode_wilayah'] == $val['id_kabupaten']){
										$namakab = $kab['nama_wilayah'];
									}else{
										$namakab = "";
									}
									?>
